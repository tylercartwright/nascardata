...
ISNULL(CharlotteTop10,0) + ISNULL(DaytonaTop10,0) + ISNULL(OcconeecheeTop10,0) + ISNULL(LanghorneTop10,0) + ISNULL(HamburgTop10,0) + ISNULL(MartinsvilleTop10,0) + ISNULL(HeidelbergTop10,0) + ISNULL(NorthWilkesboroTop10,0) AS [Top 10s]
...
LEFT JOIN [NASCAR_1949].[dbo].[Top10s] AS [Top10s] ON [Top10s].Driver = Drivers.Driver
ORDER BY [Top 10s] DESC, Driver